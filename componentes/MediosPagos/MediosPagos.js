...
function MediosPagos ({ navigation  }){
    ...
    const [datamediospagos,setDatamediospagos]=useState([])
    const [datamediospagoscompleto,setDatamediospagoscompleto]=useState([])
    const [montototalingreso,setMontototalingreso]=useState(0)
    const [canttotalmedios,setcanttotalmedios]=useState(0)
    ...
    const handlePress = () => {
        ...
        const concepto={'id':0,'nombre_medio':'','estado':1,'anotacion':''}
        navigate("MediosPagosRegistro", { concepto})
    ...
    const realizarbusqueda= (palabra)=>{
      ...
      let arrayencontrado = datamediospagoscompleto.filter(item => 
        item.nombre_medio.toLowerCase().includes(pal)
        );

      const conceptosGrupos = chunk(arrayencontrado, 2)
      setDatamediospagos(conceptosGrupos)
    }
    ...
    useEffect(() => {
            ...
            const cargardatos=async()=>{
                ...
                setGuardando(true)
                const body = {};
                const endpoint='MisMediosPagos/0/'
                ...
                if (respuesta === 200){
                    ...
                    setDatamediospagoscompleto(registros)
                    ...
                    setcanttotalmedios(cantingreso)
    ...
    }, [estadocomponente.mediospagoscomp]);
            ...
            <View style={{ flex: 1 }}>    
                        ...
                        <View style={{ flexDirection: 'row', marginBottom: 10,marginLeft:5,marginRight:5 }}>
                            {item.map(concepto => (
                                    ...
                                    <TouchableOpacity key={concepto.id} 
                                                      ...
                                                      onPress={() => {navigate('MediosPagosDetalle', { concepto });}}
                                                      >


                                        <LinearGradient key={concepto.nombre_medio} 
                                        ...
                                        >
                                          ...
                                          <View style={{flexDirection:'row'}}>
                                            ...
                                            <Text style={[styles.textoconcepto,{ marginLeft:5,color: colors.text,paddingRight:10}]}>{concepto.nombre_medio}</Text>
                                          </View>
                ...
                <View style={styles.resumencontainer}>
                    ...
                    <Text style={[styles.contenedortexto,{ color: colors.text}]}>
                        ...
                        {Number(canttotalmedios).toLocaleString('es-ES')}
                    </Text>
