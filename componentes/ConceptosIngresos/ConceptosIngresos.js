...
function ConceptosIngresos ({ navigation  }){
    ...
    const handlePress = () => {
        ...
        navigate("ConceptosIngresosRegistro", { concepto})
    ...
    const realizarbusqueda= (palabra)=>{
      ...
      let arrayencontrado = dataingresoscompleto.filter(item => 
        item.nombre_producto.toLowerCase().includes(pal)
        );
    ...
    useEffect(() => {
            ...
            const cargardatos=async()=>{
                ...
                if (respuesta === 200){
                    ...
                    let totalingreso=0
                    let cantingreso=0
                    registros.forEach(({ TotalIngresos }) => {totalingreso += TotalIngresos,cantingreso+=1})
