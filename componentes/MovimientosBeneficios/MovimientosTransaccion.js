...
function MovimientosTransaccion({ navigation }){
    ...
    const seleccionopcionmodal=(itemsel)=>{
        ...
        setEntidadsel(itemsel.opcion)
        setCodigoentidad(itemsel.id)
        toggleModal()
    ...
    const abrirgasto =()=>{
        ...
        toggleModal()
        setModalplaceholder('Buscar Entidad..')
    ...
    const registrar_egreso = async () => {
        ...
        const endpoint='RegistroMovimientoBeneficio/'
    ...
    useEffect(() => {
        ...
        const cargardatos=async()=>{
            setGuardando(true)
            const body = {};
            const endpoint='MisEntidadesBeneficios/0/'
            ...
            if (respuesta === 200){
                ...
                const itemodal=(lista.map(itemtipo => ({
                  opcion: itemtipo.nombre_entidad,
                  id: itemtipo.id
                })));
                ...
                if(item.id===0){
                  ...
                  const listadoentidades = result['data']

                  
                  
                  setEntidadsel(item.NombreEntidad)
                  setCodigoentidad(item.entidad)
                  ...
                  setMonto(item.monto)
                  setFechaegreso(item.fecha_beneficio)
                  ...
                  <ScrollView style={{padding:10,maxHeight:350,marginLeft:10,marginRight:10}}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems:'stretch' }}>
                            ...
                            <Text style={[styles.inputtextactivo, 
                                            { width:'85%',
                                            color: entidadsel ? colors.text : 'gray',
                                            borderBottomColor: entidadsel ? colors.textbordercoloractive : colors.textbordercolorinactive}]} 
                                >
                                {entidadsel ? entidadsel : 'Seleccione Entidad..'}
                      ...
                      <View style={{ flexDirection: 'row', alignItems:'stretch' }}>
                          ...
                          <Text style={[styles.inputtextactivo, 
                            ...
                            {fechaegreso ? moment(fechaegreso).format('DD/MM/YYYY') : 'Fecha Beneficio'}
...
export default MovimientosTransaccion