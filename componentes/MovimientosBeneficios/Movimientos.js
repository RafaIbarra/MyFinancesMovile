...
function Movimientos ({ navigation  }){
    ...
    const handlePress = () => {
        ...
        const item={'id':0}
        navigate("MovimientosTransaccion", { item})
    ...
    const realizarbusqueda= (palabra)=>{
      ...
      let arrayencontrado = dateegresoscompleto.filter(item => 
        item.NombreEntidad.toLowerCase().includes(pal) ||
    ...
    useEffect(() => {
        ...
        const cargardatos=async()=>{
              ...
              const body = {};
              const endpoint='MisMovimientosBeneficios/' + anno_storage +'/' + mes_storage + '/0/'
              ...
              if (respuesta === 200){
                  ...
                  let totalgasto=0
                  let cantgasto=0
                  registros.forEach(({ monto }) => {totalgasto += monto,cantgasto+=1})
                  setMontototalegreso(totalgasto)
                  setcanttotalegreso(cantgasto)
                  setGuardando(false)
                  // actualizarEstadocomponente('compmovimientos',false)
                  // actualizarEstadocomponente('datamovimientos',registros)
      ...
      }, [estadocomponente.compmovimientos]);
        ...
        <SafeAreaView style={{ flex: 1 }}>
              ...
              <View style={{ flex: 1 }}>    
                {guardando &&(<Procesando></Procesando>)}
                  ...
                  {!busqueda &&( <Text style={[styles.titulocabecera, { color: colors.text}]}>Datos Beneficios</Text>)}
                          ...
                          renderItem={({item}) =>{
                              return(
                                  <TouchableOpacity  style={[styles.contenedordatos
                                    ...
                                    onPress={() => {navigate('MovimientosDetalle', { item });}}
                                  
                                  >
                                      <View style={[styles.columna, { flex: 2 }]}> 
                                          <Text style={[styles.textocontenido,{ color: colors.text}]}> Entidad: {item.NombreEntidad}</Text>
                                          
                                          <Text style={[styles.textocontenido,{ color: colors.text}]}> Fecha Beneficio: {moment(item.fecha_beneficio).format('DD/MM/YYYY')}</Text>
                                          ...
                                          <Text style={[styles.textocontenido,{ color: colors.text}]}> Motivo: {item.anotacion}</Text>
                                      ...
                                      <View style={[styles.columna, { flex: 1,marginTop:30 }]}> 
                                          ...
                                          <Text style={[styles.textototal,{ color: colors.text,fontWeight:'bold'}]}> Gs.: {Number(item.monto).toLocaleString('es-ES')} </Text>
